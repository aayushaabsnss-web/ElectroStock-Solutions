 ...
 * Displays active and resolved low-stock alerts.
 * Uses Alert class: getActive() calls sp_getActiveAlerts,
...
<?php // Threshold configuration — Store Owner only
if(isOwner()): ?>
...
<div class="card" style="margin-bottom:16px">
  ...
  <table class="tbl">
    ...
    <tbody>
    <?php $e=true; foreach($active as $al): $e=false; $sf=max(0,$al["threshold"]-$al["current_qty"]); ?>
    <tr>
      <td class="fw"><?= h($al["product_name"]) ?></td>
      <td class="mono muted"><?= h($al["sku"]) ?></td>
      <td class="mono <?= $al["current_qty"]==0?"c-red":"c-amber" ?>"><?= $al["current_qty"] ?></td>
      <td class="mono"><?= $al["threshold"] ?></td>
      <td><span class="badge b-red">-<?= $sf ?></span></td>
      <td class="muted" style="font-size:11px"><?= date("d M Y H:i",strtotime($al["alerted_at"])) ?></td>
      <?php if(isOwner()): ?>
      <td><div style="display:flex;gap:5px;align-items:center"><a href="view.php?id=<?= $al["id"] ?>" class="icon-btn" title="View">&#128065;</a><form method="POST"><input type="hidden" name="resolve_id" value="<?= $al["id"] ?>">
        <button class="btn btn-success btn-sm">&#10003; Resolve</button></form></div></td>
      <?php endif; ?>
    </tr>
...
<div class="card">
  ...
  <table class="tbl">
    ...
    <tr><td class="fw"><?= h($al["product_name"]) ?></td>
    <td class="muted"><?= $al["resolved_at"]?date("d M Y H:i",strtotime($al["resolved_at"])):"—" ?></td>
    <td><?= h($al["resolved_by_name"]??"—") ?></td></tr>
